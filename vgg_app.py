...
Created By: t.parisotto
-------------------------------------------------------------------------------

app.py :
    1) Instantiates and compiles model defined in vgg16_base.py
            ...
            for epoch in param_grid['epochs']:
                for batch in param_grid['batch_size']:
                    for opt in param_grid['optimizers']:
                        ...
                        if model == 'baseline':                         
                            # Baseline model
                            baseline_model_path = './models/baseline/vgg_model_' + str(epoch) + activation_fn + opt + str(batch) + '_' + timestamp("%d%b_%H%M") + 'baseline.h5'
                            baseline_wts_path = './models/baseline/vgg_model_wts' + str(epoch) + activation_fn + opt + str(batch) + '_' + timestamp("%d%b_%H%M") + 'baseline.h5'
                            baseline_call_back = tf.keras.callbacks.TensorBoard(log_dir='./logs/vgg/baseline', histogram_freq=0, write_graph=True, write_images=True)
                            baseline_model = vgg_base.baseline_model(fine_tune_from=100, fcl_activation=activation_fn)
                            ...
                            batchnorm_model_path = './models/batchnorm/vgg_model_' + str(epoch) + activation_fn + opt + str(batch) + '_' + timestamp("%d%b_%H%M") + 'batchnorm.h5'
                            batchnorm_wts_path = './models/batchnorm/vgg_model_wts' + str(epoch) + activation_fn + opt + str(batch) + '_' + timestamp("%d%b_%H%M") + 'batchnorm.h5'
                            batchnorm_call_back = tf.keras.callbacks.TensorBoard(log_dir='./logs/vgg/batchnorm', histogram_freq=0, write_graph=True, write_images=True)
                            batchnorm_model = vgg_base.batch_norm_model(fine_tune_from=100, fcl_activation=activation_fn)
                            ...
                            # Dropout model
                            dropout_model_path = './models/dropout/vgg_model_' + str(epoch) + activation_fn + opt + str(batch) + '_' + timestamp("%d%b_%H%M") + 'dropout.h5'
                            dropout_wts_path = './models/dropout/vgg_model_wts' + str(epoch) + activation_fn + opt + str(batch) + '_' + timestamp("%d%b_%H%M") + 'dropout.h5'
                            dropout_call_back = tf.keras.callbacks.TensorBoard(log_dir='./logs/vgg/dropout', histogram_freq=0, write_graph=True, write_images=True)
                            dropout_model = vgg_base.dropout_model(fine_tune_from=100, fcl_activation=activation_fn)
                            ...
                            # Weight decay model
                            w_decay_model_path = './models/w_decay/vgg_model_' + str(epoch) + activation_fn + opt + str(batch) + '_' + timestamp("%d%b_%H%M") + 'w_decay.h5'
                            w_decay_wts_path = './models/w_decay/vgg_model_wts'+ str(epoch) + activation_fn + opt + str(batch) + '_' + timestamp("%d%b_%H%M") + 'w_decay.h5'
                            w_decay_call_back = tf.keras.callbacks.TensorBoard(log_dir='./logs/vgg/w_decay', histogram_freq=0, write_graph=True, write_images=True)
                            w_decay_model = vgg_base.weight_decay_model(fine_tune_from=100, fcl_activation=activation_fn)
